...
## 2026-09-22 [react] 로또 실행 중 설정 잠금 정책

- 결정: `useLottoSimulation`의 status가 `running` 또는 `paused`일 때 시뮬레이션 조건(목표 등수, 반복, 번호 방식, 번호판)은 잠근다(입력 비활성화). 1등 금액 입력과 세전/세후 토글은 실행 중에도 항상 편집 가능하며, 변경 즉시 결과의 금액 지표(누적 당첨금, 손익 등)에 반영된다.
- 이유: 실행 중인 시뮬레이션의 조건(목표 등수, 티켓 등)을 바꾸면 이미 누적된 시도와 새 조건이 섞여 결과가 무의미해진다. 반면 1등 금액·세전세후는 화면 표시 방식일 뿐 시뮬레이션 자체에 영향을 주지 않으므로 제한할 이유가 없다.
- 구현: 금액 지표는 `useLottoSimulation`이나 `protocol.ts`를 바꾸지 않고, UI 레이어에서 `summarize(state, prizeTable)`을 현재(live) `prizeTable`로 매번 다시 계산하는 방식으로 구현한다(`LottoResult.tsx`).

## 2026-09-22 [react] 로또 손익 표기(순손실 대체)

- 결정: 2026-09-21 "로또 결과 표시 원칙"의 지표 "순손실"을 "손익"으로 대체한다. 손익 = 누적 당첨금 − 누적 비용이며 부호를 붙여 표시한다(양수는 "+", 음수는 "-", 0은 부호 없음. 예: `prize.ts`의 `formatSignedWon`).
- 이유: "순손실"은 부호가 반대(비용−당첨금)라 당첨금이 비용을 넘는 경우를 "음수 손실"로 표현해야 해 읽기 어렵다. "손익"은 부호 그대로 읽을 수 있다.
- engine의 `summarize().netLoss`(= 비용 − 당첨금) 자체는 바꾸지 않는다. 표시 시점에만 부호를 반전한다(`formatSignedWon(-summary.netLoss)`).

## 2026-09-22 [react] 로또 결과 영역 구성

- 결정: 결과 영역은 상태(한국어 레이블, 완료 시 "목표 도달"/"횟수 소진" 구분) → "이 실행의 조건 요약"(시작 시점의 config 스냅샷, 실행 중 조건을 다시 바꿔도 영향받지 않음) → 누적 시도 수와 목표 등수의 기대 시도 수 병기 → 누적 비용·당첨금·손익·N년 → 1~5등 당첨 횟수 순으로 구성한다. 완료 시에는 추가로 추첨 번호와 티켓을 표시하고, 일치 번호는 색에만 의존하지 않게 굵기·배경·"●" 표시로 구분한다. 보너스 번호 일치는 본번호 일치와 다른 배경 농도와 "보너스" 텍스트 라벨로 별도 구분하며, 본번호 일치 개수와 보너스 일치 여부를 문장으로도 요약해(예: "본번호 5개 일치 · 보너스 일치") 2등처럼 판정 근거가 한눈에 보이지 않는 경우에도 왜 그 등수인지 드러나게 한다.
- 이유: "조건 요약"을 스냅샷으로 따로 두지 않으면 완료/정지 후 조건 입력을 다시 바꿨을 때 결과가 어떤 조건에서 나온 것인지 알 수 없다. 색만으로 일치 번호를 구분하면 색맹 사용자가 구분할 수 없다.

## 2026-09-22 [react] 로또 DevPanel 제거와 프로덕션 공개

- 결정: `DevPanel.tsx`를 삭제하고, `LottoPage.tsx`의 `import.meta.env.DEV` 분기를 제거해 `/lotto`와 `/embed/lotto`에서 항상 실제 화면을 렌더한다. 상태 소유자는 `LottoPage.tsx`이며, 결과 표시(`LottoResult.tsx`)와 당첨금·세금·분기점 표시(`LottoPrizeTax.tsx`)는 표시 전용 컴포넌트로 분리한다. 기존 `LottoSettings.tsx`는 `LottoPage.tsx`로 흡수한다(별도 이름으로 유지할 이유가 없어짐).
- 이유: DevPanel은 Worker/hook을 사람이 직접 조작해 검증하기 위한 임시 화면이었고, 설정 화면이 실제 시작 버튼과 연결되면서 그 역할이 끝났다. `LottoPage.tsx`가 DevPanel/LottoSettings를 조립하던 역할도 함께 사라지므로, 상태 소유를 `LottoPage.tsx` 자신으로 합쳐 불필요한 래퍼 컴포넌트를 없앤다.

## 2026-09-22 [react] 로또 Worker 예외 처리

- 결정: `worker.ts`의 `drive()` 본문을 try/catch/finally로 감싼다. `runBatch` 등이 예기치 않게 던지면 처리되지 않은 rejection으로 끝나는 대신 `{type:'error', message}`를 보내고, `current.driving`은 `finally`에서 되돌린다.
- 이유: 이전에는 Worker 안에서 발생하는 런타임 예외가 메인 스레드에 보고되지 않아 화면이 조용히 멈출 수 있었다.
